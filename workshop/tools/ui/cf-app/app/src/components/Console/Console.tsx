...
interface IConsolePromptProps {
    point?: number;
    value: string;
    label: string;
    argument?: string;
}
export class ConsolePrompt extends React.Component<IConsolePromptProps, {}> {

    static defaultProps: IConsolePromptProps = {
        point: -1,
        value: '',
        label: '> ',
        argument: null,
...
let ConsoleMessage: React.SFC<IConsoleMessageProps> = (props: IConsoleMessageProps) => {
...
export class Console extends React.Component<IConsoleProps, IConsoleState> {
    ...
    keyDown = (e: any) => {
        ...
        const ctrlXCodes: IKeyMap = { // TODO state
            // C-x Rubout
        ...
        const ctrlShiftCodes: IKeyMap = {
            // C-_ TODO
            //189: this.undo,
            // C-@ TODO
            //50: this.setMark,
        };
        const metaCodes: IKeyMap = {
            // M-f
            70: this.forwardWord,
            // M-b
        ...
        let metaShiftCodes: IKeyMap = { // TODO hook in
            // M-<
    ...
    consoleInsert = (insert: string, replace: number = 0): IConsoleState => {
    ...
    searchHistory = (direction: SearchDirection = this.state.searchDirection, next: boolean = false): IConsoleState => {
    ...
    render() {
        ...
        >
            ...
            {this.state.log.map((val: ILogEntry) => {
                return [
                    // tslint:disable-next-line:jsx-key
                    <ConsolePrompt label={val.label} value={val.command} />,
                    ...val.message.map((val: ILogMessage, idx: number) => {
