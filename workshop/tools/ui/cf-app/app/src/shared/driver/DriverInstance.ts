...
import { DRIVER_FLAGS, EVENTS, RESPONSE_SETTINGS, COMMAND_SETTINGS, RESPONSE_MODE } from '../types';
 ...
 */
export class DriverInstance extends Model {

    public service: DeviceService;
    public device: Device;
    constructor(device: Device, service: DeviceService) {
        super();
    ...
    public onDeviceMessage(message: any) {
        const self = this;
        // console.debug('device instance, message : ', message);
        const ab: ArrayBuffer = message.data;
    }

    init() {
        this.service.socket.on('onDeviceMessage', this.onDeviceMessage);
    }
     ...
     */
    getLineBreak() {
        if (!this.responseSettings || this.responseSettings.responseMode!== RESPONSE_MODE.DELIMITER) {
            return '';
        }
    ...
    split(str) {
        ...
        if (this.responseSettings.responseMode === RESPONSE_MODE.DELIMITER) {
    ...
    onMessageRaw(data) {
        ...
        if (responseSettings && responseSettings.responseMode === RESPONSE_MODE.DELIMITER && responseSettings.delimiter && responseSettings.delimiter.length > 0) {
    ...
    setVariable(title, value, save, publish, highlight) {
        ...
        const _variable = this.device.scope.getVariable(title);
        if (_variable) {
