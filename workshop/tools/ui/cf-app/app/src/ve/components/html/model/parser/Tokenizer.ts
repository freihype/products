    ...
    static stringStream(source) {
        ...
        return {
            ...
            next() {
                if (!ensureChars()) {
                    if (accum.length > 0) {
                        throw new Error('End of stringstream reached without emptying buffer (\'' + accum + '\').');
                    } else {
                        throw StopIteration;
                    }
                }
            ...
            lookAhead(str, consume, skipSpaces, caseInsensitive) {
                ...
                while (true) {
                    ...
                    if (end <= current.length) {
                        ...
                        try { current = source.next(); } catch (e) { if (e != StopIteration) { throw e; } break; }
                        pos = 0;
                        str = str.slice(left);
                    } else {
                        break;
            ...
            lookAheadRegex(regex, consume) {
                ...
                const matched = current.slice(pos).match(regex);
                if (matched && consume) {
