...
export const CSSParser = (function () {
    let tokenizeCSS = (function () {
        ...
        return function (source, startState?: any) {
    ...
    function parseCSS(source, basecolumn?: any) {
        basecolumn = basecolumn || 0;
        let tokens = tokenizeCSS(source);
        // tslint:disable-next-line:one-variable-per-declaration
        let inBraces = false, inRule = false, inDecl: any = false;

        let iter = {
            next: function () {
                // tslint:disable-next-line:one-variable-per-declaration
                let token = tokens.next(), style = token.style, content = token.content;
                ...
                if (content == '{' && (inDecl as string) === '@media') {
                    inDecl = false;
                } else if (content == '{') {
                    inBraces = true;
                } else if (content == '}') {
            ...
            copy: function () {
                // tslint:disable-next-line:one-variable-per-declaration
                let _inBraces = inBraces, _inRule = inRule, _tokenState = tokens.state;
...
CSSParser['parse'] = function (text, parentElement) {
    ...
    let parser = CSSParser.make(stream);
    let token;
    let selector;
    let combined;
    let combiner = ' ';
            ...
            switch (token.style) {
                ...
                case 'css-select-op':
                    ...
                    while (nextToken().content != '}') {
                        let nameOffset = token.offset;
                        let propertyName = token.content;
                        let skipNext = false;
                        let property = new CSSProperty();
                        if (token.type == 'css-hash') {
                            ...
                            if (token.content != '*') { // is probably bad syntax,
                                // but dojo.css has "
                                // *font-size "
                                error('expecting identifier around ' + selector.getText() + '{ ' + property.name + ': ' + property.value);
                            } else {
                                nextToken();
                                propertyName += token.content;
                            }
                        ...
                        if (!skipNext) {
                            if (nextToken().content != ':') {
                                error('expecting \':\' ' + selector.getText() + '{ ' + property.name + ': ' + property.value);
                            }
                        }
                        nextToken();
                ...
                case 'css-at':
                    let ruleName = token.content.substring(1);
                    let atRule = (ruleName == 'import') ? new CSSImport()
                        : new CSSAtRule();
