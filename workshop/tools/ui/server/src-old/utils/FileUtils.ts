...
export function pathinfo(path: string, options?: EPATH_PARTS | any): any {
    //  discuss at:
    // original by: Nate
    ...
    let have_basename: string | boolean = false,
        have_extension: string | boolean = false,
        have_filename: string | boolean = false;
    ...
    // Internal Functions
    function __getExt(path): string | boolean {
        var str = path + '';
    ...
    if (options & OPTS.PATHINFO_FILENAME) {
        ...
        if (false === have_filename) {
            have_filename = have_basename.slice(0, have_basename.length - (have_extension ? (<string>have_extension).length + 1 :
